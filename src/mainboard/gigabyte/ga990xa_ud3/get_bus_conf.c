...
//Based on asus m5a88-v SB and amd dinar NB

/* Global variables for MB layouts and these will be shared by irqtable mptable
 * and acpi_tables busnum is default.
 */
u8 bus_isa;
u8 bus_rd980[14]; //bus_rd980
u8 bus_sb800[6];
u32 apicid_sb800;
...
void get_bus_conf(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(bus_rd980); i++) {
		bus_rd980[i] = 0;
	}
	...
	bus_rd980[0] = (pci1234x[0] >> 16) & 0xff;
	bus_sb800[0] = bus_rd980[0];
	...
	/* rd890 */
	for (i = 1; i < ARRAY_SIZE(bus_rd980); i++) {
		dev = dev_find_slot(bus_rd980[0], PCI_DEVFN(sbdn_rd980 + i, 0));
		if (dev) {
			bus_rd980[i] = pci_read_config8(dev, PCI_SECONDARY_BUS);
			if(255 != bus_rd980[i]) {
				bus_isa = pci_read_config8(dev, PCI_SUBORDINATE_BUS);
				bus_isa++;
				bus_type[bus_rd980[i]] = 1; /* PCI bus. */
			}
		}
	}
